...
var beginYear = window.localStorage.carbonActual;
var endYear = window.localStorage.carbonTotal;
var step = 7;
var delta = (endYear-beginYear)/step
var lineCount = 10;
...
function makeSeriesData(year, negative) {
    ...
    var r = ((year - beginYear + 1)/delta) * 10;
    var seriesData = [];
...
setInterval(function () {
    ...
    if (currentYear>endYear&&currentYear<endYear+10){
        myChart.setOption({
            xAxis: {
                name: 'Vous pourriez faire plus de contribution au delà de ' + endYear + " g"
            },
        });
    }
    if(currentYear<beginYear){
        myChart.setOption({
            xAxis: {
                name: 'Avant votre participation'
            },
        });
    }
}, 800);
